...
void printParseTree(parseTreeNode* root, char** tokenMap, char** non_terminals, FILE* outputfile) {
  if(root==NULL)
    return;
    ...
    if(root->children != NULL) {
        isLeaf = 0;
        printParseTree(root->children, tokenMap, non_terminals, outputfile); 
    }
    ...
    if(tmp != NULL) {
        ...
        for(; tmp!=NULL;)
        {
            printParseTree(tmp, tokenMap, non_terminals, outputfile);
            tmp = tmp -> nextSibling;
        }
    }
}